...
export const ProductFeed = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!products || products.length === 0) {
          await fetch(
            `https://${projectId}.supabase.co/functions/v1/make-server-d588a8d5/seed-products`,
            {
              method: 'POST',
              headers: {
                'Authorization': `Bearer ${publicAnonKey}`
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d588a8d5/products?${params}`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`
          }
  ...
  const filterAndSortProducts = () => {
    ...
    // Sort products (server already handles filtering)
    switch (sortBy) {
      case 'newest':
